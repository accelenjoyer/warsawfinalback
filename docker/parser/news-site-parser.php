...
class News_Site_Parser {
    ...
    public function process_news_sources() {
        $this->log("Entered process_news_sources");
        ...
        try {
            $sources = get_option('news_sources', array());
            $this->log("Sources count: " . count($sources));
            if (!is_array($sources) || count($sources) === 0) {
                throw new Exception("Sources not found or empty");
            }
    
            foreach ($sources as $source_url => &$source_data) {
                $this->log("Checking source $source_url with status {$source_data['status']}");
                ...
                if (!$result) {
                    ...
                    $this->log("Processing failed for $source_url, error count: {$source_data['error_count']}");
                    if ($source_data['error_count'] >= 3) {
                        $source_data['status'] = 'error';
                        $this->log("Source $source_url marked as error");
                    }
                } else {
                    $source_data['status'] = 'in_progress';
                    $this->log("Source $source_url status set to in_progress");
                }
...
add_action('init', function () {
    if (isset($_GET['force_parser']) && current_user_can('manage_options')) {
        $parser = run_news_site_parser();
        $parser->process_news_sources();
        exit('Parser executed manually.');
    }
});