...
class News_Parser_Processor
{
    ...
    private function process_post($post, $source_url, $post_type)
    {
        try {
            ...
            // Отправляем статью в Node.js backend
            try {
                $api_url = "http://app:3000/adminmenu"; // имя сервиса app из docker-compose
                $article_data = array(
                    'title' => $post_title,
                    'rawContent' => $post_content,
                    'author' => get_option('news_parser_default_author', 1),
                    'categories' => $post['_embedded']['wp:term'][0] ?? [],
                    'images' => !empty($post['_embedded']['wp:featuredmedia'][0]['source_url'])
                ...
                if (is_wp_error($response)) {
                    $this->log("Ошибка отправки статьи в backend: " . $response->get_error_message(), 'error');
                } else {
                    $this->log("Статья успешно отправлена в backend: " . $post_title);
                }
            } catch (Exception $e) {
                $this->log("Ошибка связи с backend: " . $e->getMessage(), 'error');
            }
